...
export async function POST(req: Request) {
  try {
    ...
    // includes multi-lines, junctions etc; sum their per-line totals
    ...
    // You said you don't need extras totals as a flat column.
    ...
    const public_id = makeId(10);
    const public_token = makeId(24);
    ...
    const { data, error } = await supabase
      .from(QUOTES_TABLE)
      .insert(insertRow)
      .select("public_id, public_token")
      .single();

    if (error) {
      console.error("Supabase insert error:", error);
      return Response.json({ error: "Failed to save quote", detail: error.message }, { status: 500 });
    }

    const base = PUBLIC_QUOTE_BASE_URL ? PUBLIC_QUOTE_BASE_URL.replace(/\/$/, "") : "";
    const url = `${base}/q/${data.public_id}?t=${data.public_token}`;

    return Response.json(
      { ok: true, public_id: data.public_id, publicUrl: url },
      { status: 200 }
    );
  } catch (err: any) {
